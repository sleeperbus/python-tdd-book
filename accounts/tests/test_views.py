...
class SendLoginEmailViewTest(TestCase):
    ...
    @patch('accounts.views.send_mail')
    def test_sends_link_to_login_using_token_udi(self, mock_send_mail):
        self.client.post('/accounts/send_login_email', data={'email': 'edith@example.com'})
        token = Token.objects.first()
...
class LoginViewTest(TestCase):
    ...
    def test_calls_authenticate_with_udi_from_get_request(self, mock_auth):
    ...
    def test_calls_auth_login_with_user_is_there_is_one(self, mock_auth):
        response = self.client.get('/accounts/login?token=abcd123')
        self.assertEqual(
